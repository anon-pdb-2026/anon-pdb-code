#!/bin/bash
# Bug generation with 3 models on both datasets IN PARALLEL, then merge.
#
# Generators: openai/gpt-5.1-codex, gemini/gemini-2.5-pro, anthropic/claude-sonnet-4-5-20250929
# All use temperature=1.0 and max_tokens=32000.
# bug_per_time=20, max_bugs=4, max_gen_per_bin=5.
#
# NOTE: [design thought] Each (model, dataset) job is independent — different API
# endpoint and different output directory — so we fan out all 6 jobs concurrently,
# wait for them, then do per-dataset merge serially. Per-job logs land in
# bash_log/ so you can tail them during the run.
set -e
...
echo "=== Launching 6 parallel generation jobs ==="
declare -a PIDS=()
  ...
  for i in "${!MODELS[@]}"; do
    ...
    (
      $PYTHON src/bug_generation.py \
        ...
        --output_prefix "${prefix}_buggy_code" \
        --bug_per_time 20 \
        --max_bugs 4 \
        --max_gen_per_bin 5 \
        --temperature 1.0 \
...
# NOTE: [design thought] We don't `set -e` through `wait` — collect per-job exit
# codes manually so one failure still lets the others finish, and we can report
# which job failed.
fail=0
for pid in "${PIDS[@]}"; do
  if ! wait "$pid"; then
...
for dataset in "${DATASETS[@]}"; do
  ...
  GEN_FILES=$(ls "results/$dataset/bug_data/"*_buggy_code_*.json 2>/dev/null | xargs -n1 basename)
  if [[ -z "$GEN_FILES" ]]; then
    echo "ERROR: No generated files found for $dataset"
    exit 1
  fi
  ...
  $PYTHON src/merge.py \
    ...
    --in_files $GEN_FILES \
    --output_file "results/$dataset/bug_data/${dataset}_pdb_single.json" \
    --samples_per_group 5 \
    --max_lines_per_block 1 \
    --min_lines_per_block 1 \
    --stride 2
...
data = json.load(open('results/$dataset/bug_data/${dataset}_pdb_single.json'))
...
echo ""
echo "=== Generation + Merge Complete ==="
